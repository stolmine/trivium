...
Last Updated: 2025-11-21 (Phase 30: Flashcard Manager - Phase 3 Complete + Styled Checkboxes ✅)
...
**Description**: Complete implementation documentation for Phase 30 Flashcard Manager - Phases 1-3 (Core Table + Filtering & Sorting + Batch Operations) + Post-Phase 3 Improvements. Phase 1 establishes foundation with dual-pane layout (left: 16-column table, right: detail view), TanStack Table v8 integration, pagination system (50 cards/page), and backend infrastructure. Phase 2 adds powerful filtering and sorting: text search with 300ms debounce, 5 mutually exclusive quick filters, advanced filter panel with date defaults (earliest card to today), single-column sorting (3-state toggle), row click selection with header checkbox, collapsible detail panel (Cmd/Ctrl+I), filter count badge, column visibility control with master toggle, all state persists to localStorage. Phase 3 implements comprehensive batch operations: delete multiple cards with confirmation dialog, duplicate cards with reset FSRS data, bury cards (set buried_until), reset FSRS statistics, change card learning states, BulkActionToolbar component, keyboard shortcuts (Delete, Cmd/Ctrl+D), platform-aware tooltips, transaction-safe operations. **Post-Phase 3**: Styled shadcn/ui Checkbox component replacing native HTML checkboxes for better theme consistency and dark mode support, indeterminate state for "Toggle All", hiddenColumns Set serialization bug fix (Set → Array → Set) preventing localStorage corruption. **Backend**: Three new commands (batch_delete_flashcards, duplicate_flashcards, batch_update_flashcards), FlashcardBatchUpdate type, dynamic SQL query building, recursive folder filtering with CTE. **Frontend**: BulkActionToolbar, BatchDeleteDialog, BatchOperationsDialog, Checkbox UI component, all operations refresh table and clear selection. **Files**: Phase 1 (7 created, 6 modified), Phase 2 (3 created, 5 modified), Housekeeping (7 modified), Column Visibility (1 created, 3 modified), Phase 3 (3 created, 4 modified), Styled Checkbox (1 created, 3 modified), Total 33 files. **Dependencies**: @tanstack/react-table, @radix-ui/react-checkbox. **Status**: Phase 3 Complete ✅ + Post-Phase 3 Improvements ✅ - ~35-42 hours implementation, Phase 4 (inline & modal editing) planned next
**Last Updated**: 2025-11-21
...
**Description**: Comprehensive keyboard shortcuts reference organized by feature area (Global Navigation, Reading View, Review Session, Create Cards, Ingest, Library Navigation, Flashcard Manager, Modal Dialogs). Documents all 60+ keyboard shortcuts with Windows/Linux and macOS variants, conflict resolution notes (Ctrl+1-4 vs Alt+1-3), platform consistency guidelines, accessibility features, and troubleshooting tips. Includes version history tracking shortcut changes. **Latest**: Complete Flashcard Manager section with Delete/Backspace (batch delete), Cmd/Ctrl+D (duplicate), Cmd/Ctrl+A (select all), Cmd/Ctrl+F (focus search), Cmd/Ctrl+I (toggle detail panel), Cmd/Ctrl+8 (open manager)
**Last Updated**: 2025-11-21
