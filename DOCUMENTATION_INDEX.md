...
Last Updated: 2025-11-21 (Phase 30: Flashcard Manager - Phase 2 Complete ✅)
...
**Description**: Complete implementation documentation for Phase 30 Flashcard Manager - Phases 1-2 (Core Table + Filtering & Sorting) + Post-Phase 2 Housekeeping + Column Visibility Feature. Phase 1 establishes foundation with dual-pane layout (left: 16-column table, right: detail view), TanStack Table v8 integration, pagination system (50 cards/page), and backend infrastructure. Phase 2 adds powerful filtering and sorting: text search with 300ms debounce, 5 mutually exclusive quick filters, advanced filter panel with date defaults (earliest card to today), single-column sorting (3-state toggle), row click selection with header checkbox, collapsible detail panel (Cmd/Ctrl+I), filter count badge, all state persists to localStorage. **Post-Phase 2 Housekeeping** (8 fixes): Single-column sorting (simplified from multi-column), date filter defaults, row selection with auto-clear on filter change, mutually exclusive quick filters, Cmd/Ctrl+A fix for search field, database path fix (trivium_dev.db → trivium.db), sort parameter pipeline fix. **Column Visibility Feature**: Dropdown menu with checkboxes for all 16 columns, master toggle all with indeterminate state support, Select column non-hideable, menu stays open during selection, persistent state across sessions, survives quit/relaunch cycles. **Backend**: FlashcardFilter struct (17 fields), single SortField parameter, dynamic SQL query building, recursive folder filtering with CTE, date defaults query. **Frontend**: Enhanced flashcardManagerStore with selection and columnVisibility state, FlashcardTableToolbar, FlashcardFilterPanel, ColumnVisibilityMenu components. **Critical Bug Fix**: Value change check in setFilter prevents infinite re-render loops. **Files**: Phase 1 (7 created, 6 modified), Phase 2 (3 created, 5 modified), Housekeeping (7 modified), Column Visibility (1 created, 3 modified), Total 29 file changes. **Status**: Phase 2 + Housekeeping + Column Visibility Complete ✅ - ~28-34 hours implementation, Phase 3 (multi-select & batch operations) planned next
**Last Updated**: 2025-11-21
...
**Description**: Comprehensive keyboard shortcuts reference organized by feature area (Global Navigation, Reading View, Review Session, Create Cards, Ingest, Library Navigation, Modal Dialogs). Documents all 60+ keyboard shortcuts with Windows/Linux and macOS variants, conflict resolution notes (Ctrl+1-4 vs Alt+1-3), platform consistency guidelines, accessibility features, and troubleshooting tips. Includes version history tracking shortcut changes. **Latest**: Cmd/Ctrl+I for toggling detail panel (Library and Flashcard Manager)
**Last Updated**: 2025-11-21
