...
### `/Users/why/repos/trivium/LIBRARY_PAGE_PLAN.md`
**Description**: Comprehensive planning document for the Library Page feature showing vision, progress, and roadmap. Documents original Mac Finder-style dual-pane vision with full CRUD capabilities, multi-selection support, info panel, and preview functionality. Breaks down all 7 phases with detailed feature lists, time estimates, implementation plans, and success criteria. **Phase Breakdown**: Phase 1 (Core Layout ✅), Phase 2 (Multi-Selection ✅), Phase 3 (View Modes ✅) + Post-Phase 3 Improvements ✅ (theme-aware icons, drag-and-drop in grid/list, URL navigation, up button/shortcut, sticky root zones, repositioned toolbar), Phase 4 (Info Panel ✅ + Polish ✅ + Search Highlighting ✅ - 3-4 hours core + 2-3 hours polish + 2-3 hours search), Phase 5 (Smart Preview Panel ✅ - Complete, ~3 hours implementation + field naming fix + housekeeping improvements), Phase 6 (Batch Operations ✅ - Complete, ~4-5 hours implementation with 4 bug fixes), Phase 7 (Keyboard Navigation & Accessibility ✅ - Complete, ~20-25 hours implementation). **Current Status**: ALL 7 PHASES COMPLETE (100%) ✅, ~50-60 hours total invested. **Technical Architecture**: Zustand state management, context-aware components, Set-based multi-selection, percentage-based layout, @dnd-kit drag-and-drop, React Router URL navigation, CTE-based recursive folder statistics, recursive search highlighting with ancestor chain illumination, smart excerpt selection (unread/current/beginning modes) with camelCase field naming, context-aware folder creation with subfolder support, batch operations with transaction safety (move, delete, export with recursive folder support), comprehensive keyboard navigation across all view modes with ARIA attributes and WCAG AA compliance. Serves as master planning reference for the entire Library Page overhaul project
**Last Updated**: 2025-11-11

### `/Users/why/repos/trivium/PHASE_29_LIBRARY_PAGE.md`
**Description**: Complete implementation documentation for Phase 29 Library Page - Dual-Pane Layout (Parts 1-6 of 7) with drag-to-root bug fix, view modes, post-phase improvements, additional grid view improvements, info panel, polish improvements, search highlighting, smart preview panel (complete), Phase 5 housekeeping improvements, selection performance investigation, and batch operations. **Status**: Phases 1-6 complete - Core dual-pane layout with resizable divider, Mac-style multi-selection, context-aware focus tracking, three view modes with full drag-and-drop and URL navigation, comprehensive info panel with statistics, smart preview panel with intelligent excerpt selection (unread/current/beginning modes) and camelCase field naming, professional polish including info view collapse, fixed action buttons, header consolidation, sidebar buttons, location persistence, removed focus styling, and intelligent search highlighting with recursive folder matching. **Phase 5 Housekeeping**: Preview label simplified to "Preview:" (removed dynamic labels), smart sentence boundary detection (100-char lookback, period-only breaks), InfoView background colors changed to bg-sidebar for dark mode consistency, context-aware folder creation with subfolder dialogs. **Phase 1 Features**: Dual-pane layout foundation (left: LibraryTree, right: info panel placeholder), resizable divider with 25-75% range and 4px width, persistent pane sizing via localStorage, view mode state management (tree/icon/list), smooth drag interactions with visual feedback, missing @tauri-apps/plugin-os package installation. **Phase 2 Features**: Mac Finder-style multi-selection (highlight-only, no checkboxes), Ctrl/Cmd+click toggle selection, Shift+click range selection, SelectionToolbar showing count and clear button, decoupled sidebar and library page state (separate selections), separate folder expand/collapse state for sidebar vs library, sync selection setting (default: false, configurable in Settings), root drop zone for moving items to top level, context-aware behavior (sidebar vs library). **Phase 29.3 Features**: Focus tracking system (route-aware, only on /library page), three focus contexts (sidebar, library-left, library-right), click-to-focus interaction, persistent focus state via localStorage, context-aware hotkeys (Ctrl+Shift+E for expand/collapse, Shift+Ctrl+F for search), visual feedback system (focused: 2px darker border, shadow, lighter bg; unfocused: 1px border, dimmed 88% opacity), independent search states (sidebar vs library with separate query/filters/matches), independent selection states (already from Phase 2). **Phase 3 Features**: Three view modes (Tree, Icon/Grid, List), ViewModeToggle component (3-button toggle with icons), BreadcrumbNav component (folder path navigation), IconGridView component (responsive CSS Grid), ListView component (sortable table with 5 columns: Name, Size, Modified, Progress, Flashcards), state management updates (currentFolderId, sortColumn, sortDirection), persistent view mode via localStorage. **Post-Phase 3 Improvements** (7 enhancements): (1) Theme-aware icon colors (removed hardcoded colors), (2) Full drag-and-drop in grid/list views (DndContext, draggable items, droppable folders, DragOverlay), (3) URL-based navigation with browser back/forward support (?folder=id), (4) "Up one level" button with Cmd/Ctrl+↑ shortcut, (5) Root drop zones with fixed visibility (only in subfolders) and sticky positioning, (6) SelectionToolbar moved to bottom of left pane, (7) Updated KEYBOARD_SHORTCUTS.md. **Additional Grid View Improvements** (4 refinements): (1) Click-in-void to deselect (clicking empty grid space clears selection), (2) Dynamic grid reflow on pane resize (CSS Grid auto-fill replaces fixed breakpoints), (3) Fixed selection count bug (range selection now scoped to visible items only via optional visibleItemIds parameter), (4) RootDropZone click propagation fix (stopPropagation prevents selection clearing). **Phase 4 Features**: Backend statistics commands (get_text_statistics, get_folder_statistics with recursive aggregation via CTEs), TextInfoView component (comprehensive text metadata, reading progress, flashcard breakdown with retention rate calculation, created/modified/last read dates with Intl.DateTimeFormat formatting), FolderInfoView component (recursive folder statistics), MultiSelectInfoView component (aggregate statistics for multiple selections with batch action placeholders), RightPane conditional rendering (dynamic component switching based on selection state), type system (TextStatistics, FolderStatistics interfaces). **Polish Improvements** (6 enhancements): (1) Info view collapse button with Cmd/Ctrl+I hotkey and localStorage persistence - left pane expands to 100% width when collapsed, (2) Fixed action button clickability in TextInfoView, (3) Header button consolidation with LibraryHeader component containing all controls (Ingest, Search, Sort, Expand/Collapse All, New Folder) and tripartite view mode toggle (Mac Finder-style), all buttons icon-only with platform-aware tooltips, (4) Sidebar buttons restored - library controls available in BOTH sidebar and library header with new setting "Show Library Controls in Sidebar" (default: true) in Settings → Defaults tab, (5) Location persistence - currentFolderId persists across navigation and restores last folder location when returning to /library, (6) Removed info view focus styling - right pane no longer has focus-related CSS classes. **Phase 29.5 Search Highlighting**: Intelligent search highlighting for Grid and List views with view-specific behavior (Tree narrows/filters, Grid/List show all with highlights), recursive folder matching (folders highlighted if name matches OR contains matching texts at any depth), ancestor chain illumination (entire parent hierarchy highlighted when deeply nested text matches), yellow color scheme (bg-yellow-100 light, bg-yellow-900/20 dark, ring-2 ring-yellow-400 in grid), CSS class ordering fix (highlight classes after selection classes for visibility), folderContainsMatches() recursive helper with short-circuit optimization, case-sensitive and whole-word search support, edge cases handled (empty query, no matches, deep nesting, selection+highlight), performance < 5ms recursive checking and < 10ms render. **Performance Investigation** (Phase 29.X): Comprehensive 6-8 hour investigation into selection lag, 9 optimizations attempted (FolderContextMenu effect storm fix, React.memo on 4 component types, debug log removal from 8 files, CSS transition removal from focus panes saving 150ms, scroll behavior change from smooth to auto, box-shadow replacement with border, CSS containment for rendering isolation, GPU acceleration with will-change hints, MultiSelectInfoView infinite loop fix). **Status**: Issue persists despite optimizations, root cause identified as cumulative from many small sources (React re-render overhead, Zustand subscriptions, DOM updates). Future options documented: virtual scrolling, state debouncing, web workers, alternative state library. **Post-Phase 2 Bug Fix**: Drag-to-root zone collision detection fix - extracted RootDropZone to separate component (fixes @dnd-kit anti-pattern), added MeasuringStrategy.Always for conditional rendering, implemented combined collision detection (pointerWithin + closestCenter). **Phase 6 Features** (Complete ✅): Backend batch operations (move_multiple_items, delete_multiple_items, export_texts with recursive folder export), three dialog components (BatchMoveDialog for folder picker, BatchDeleteDialog with confirmation, ExportDialog for text export), keyboard shortcuts (Ctrl+A for select all context-aware, Delete for batch delete), recursive folder export functionality, 4 bug fixes (select all context-aware fix, text ID type fix for exports, visual selection highlighting with bg-sidebar-primary/20, recursive folder export loop fix). **Files**: Phase 1: 5 created + 4 modified. Phase 2: 1 created + 7 modified. Phase 29.3: 2 created (focusContext.ts, useContextualHotkeys.ts) + 15 modified. Phase 3: 4 created (ViewModeToggle.tsx, BreadcrumbNav.tsx, IconGridView.tsx, ListView.tsx) + 2 modified (library.ts, LeftPane.tsx). Post-Phase 3: 7 modified. Additional improvements: 4 modified. Phase 4: 5 created (library_statistics.rs, statistics.ts, TextInfoView.tsx, FolderInfoView.tsx, MultiSelectInfoView.tsx) + 5 modified. Polish improvements: 1 created (LibraryHeader.tsx) + multiple modified. Phase 29.5: 2 modified (IconGridView.tsx, ListView.tsx). Phase 5 housekeeping: 7 modified. Phase 6: 4 created (batch_operations.rs, BatchMoveDialog.tsx, BatchDeleteDialog.tsx, ExportDialog.tsx) + 7+ modified. Performance investigation: ~15 files modified + 4 investigation documents created. Bug fix: 1 created (RootDropZone.tsx) + 1 modified (LibraryTree.tsx). **Architecture**: Percentage-based layout, Set<string> for selectedItemIds, context prop pattern ('sidebar' | 'library'), separate expand state per context, dual search context state, focus context store with route awareness, CSS-only visual feedback, React Router URL navigation, recursive folder search with ancestor chain highlighting, context-aware folder creation, batch operations with transaction safety (SQLite transactions ensure all-or-nothing operations), recursive folder export with automatic directory creation. **Next Phase**: Polish & UX (Phase 7). **Performance**: Phase 1: < 5ms render, 60 FPS resize. Phase 2: < 1ms selection toggle, < 5ms range selection (100 items). Phase 29.3: < 5ms focus change, 150ms smooth transitions, < 1ms search context ops. Phase 3: < 20ms view switch, < 40ms list render (100 items), < 5ms sort. Phase 4: < 50ms text stats, < 100ms folder stats. Phase 29.5: < 5ms recursive folder checking, < 10ms render with highlights. Phase 6: < 500ms batch operations for 100 items. Implementation time: ~4 hours (Phase 1) + ~4-6 hours (Phase 2) + ~6-8 hours (Phase 29.3) + ~5-6 hours (Phase 3) + ~2-3 hours (post-phase improvements) + ~1 hour (additional grid view improvements) + ~3-4 hours (Phase 4) + ~2-3 hours (polish improvements) + ~2-3 hours (search highlighting) + ~3 hours (Phase 5) + ~1 hour (Phase 5 housekeeping) + ~4-5 hours (Phase 6 with bug fixes) + ~1 hour (bug fix) + ~6-8 hours (performance investigation)
**Last Updated**: 2025-11-11

### `/Users/why/repos/trivium/PHASE_29.3_FOCUS_TRACKING.md`
**Description**: Complete implementation documentation for Phase 29.3 focus tracking and search/selection decoupling with settings toggle. Introduces context-aware focus tracking for Library page with visual feedback, independent search states between sidebar and library, and context-aware hotkeys. **Settings Control**: New `enable_focus_tracking` setting (default: OFF) allows users to disable click-to-focus behavior - when OFF, library always gets hotkeys and no visual outlines; when ON, full focus system with visual feedback. **Focus Tracking System**: Route-aware (only on /library page), three focus contexts (sidebar, library-left, library-right), click-to-focus interaction, persistent state via localStorage with Zustand. **Context-Aware Hotkeys**: Ctrl+Shift+E for expand/collapse all folders in focused context, Shift+Ctrl+F for opening search in focused context, route-aware (defaults to sidebar when not on library page), cross-platform (Cmd on macOS, Ctrl on Windows/Linux). **Visual Feedback**: CSS variables for light/dark modes, focused panes (2px darker border, subtle shadow, lighter bg, 100% opacity), unfocused panes (1px light border, no shadow, dimmed bg, 88% opacity), 150ms smooth transitions with cubic-bezier easing, respects prefers-reduced-motion. **Independent Search States**: Sidebar search and library search with separate query, filters (case-sensitive, whole-word), and match tracking via restructured librarySearch store (sidebar/library contexts). **Independent Selection States**: Already implemented in Phase 29.2 (sidebar: single selection, library: multi-selection with separate expand state). **Architecture**: Focus context store (focusContext.ts) with route-awareness helpers, contextual hotkeys hook (useContextualHotkeys.ts) with effective context computation, search state decoupling via context parameter pattern, CSS-only visual feedback with theme-responsive variables. **Files**: 2 created (focusContext.ts, useContextualHotkeys.ts), 15 modified (librarySearch.ts, LibrarySearchBar.tsx, index.css, Sidebar.tsx, LeftPane.tsx, RightPane.tsx, AppShell.tsx + 8 others). **Performance**: Focus change < 5ms, CSS transitions 150ms GPU-accelerated, search context ops < 1ms, localStorage < 1ms. **Known Limitations**: No keyboard focus switching (click-only), library right pane empty (Phase 4-5), debug logging active. Implementation time: ~6-8 hours
**Last Updated**: 2025-11-09

### `/Users/why/repos/trivium/SETTINGS_QUICK_REFERENCE.md`
**Description**: Quick reference guide for Settings Menu providing condensed overview for developers. Includes implemented settings list (show_links_by_default, enable_focus_tracking), settings structure (key-value pairs), keyboard shortcuts (Ctrl+6 navigation, Ctrl+L toggle), backend commands reference, frontend components overview, and common usage patterns. Designed as companion to full Phase 19 documentation for rapid lookups during development
**Last Updated**: 2025-11-09
...
**Description**: Sentence boundary detection utilities for text editing and context extraction. Enhanced `isAbbreviation()` function with single-letter abbreviation detection, multi-letter acronym support (U.S., U.K., P.S.), 70+ common abbreviations, and parenthetical abbreviation support (r., b., d., c., fl.). Functions for finding sentence starts/ends, detecting punctuation, and extracting complete sentences around selections. Used for focus/typewriter mode, mark context display, and smart editing
**Last Updated**: 2025-10-25

#### `/Users/why/repos/trivium/SENTENCE_BOUNDARY_IMPROVEMENTS.md`
**Description**: Complete implementation documentation for sentence boundary detection improvements (Phase 26 + parenthetical abbreviation fix). Covers the problem (false breaks at abbreviations), three major enhancements (single-letter abbreviations, multi-letter acronyms, expanded abbreviation list from 30 to 70+ entries), parenthetical abbreviation support (r., b., d., c., fl.), implementation details with detection order, test results (17/17 passing), and impact on focus/typewriter mode
**Last Updated**: 2025-10-25
...
**Description**: ContentEditable-based inline text editor component for selection-based editing. Activated via Ctrl+E or Edit button. Features: plain text editing, paste sanitization, auto-focus, visual state transitions, save/cancel operations, cursor preservation using Selection API (refs for cursor node/offset, restoration after React re-renders, edge case handling for emoji/surrogate pairs). Uses conditional rendering with ReadHighlighter
**Last Updated**: 2025-10-25
...
**Description**: Textarea-based text editor component providing basic editing functionality. Handles text input, change events, and content synchronization. Features cursor preservation using setSelectionRange and localStorage persistence. Currently unused but maintained for completeness. Part of the inline editing system infrastructure
**Last Updated**: 2025-10-25
